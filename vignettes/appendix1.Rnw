...
@
(you will need to install the \code{devtools} package from any
standard CRAN repository first).
...
<<beta_ex1,fig.width=12,out.width="1.2\\textwidth">>=
library(betararef)
set.seed(101)
b0 <- betasim()
plot(b0)
...
the same numbers:
<<mixplot,fig.width=12,out.width="1.2\\textwidth">>=
plot(betasim(p.mix=1))
@

We can of course allow partial mixing:
<<partmix,fig.width=12,out.width="1.2\\textwidth">>=
plot(betasim(p.mix=0.5))
@

Now we add Poisson randomness:
<<poissmix,fig.width=12,out.width="1.2\\textwidth">>=
plot(betasim(p.mix=0.5,rand="poisson"))
@

We can change the number sites or the number of abundance classes: by default, the \code{spcat} (number of species per site per abundance category) will be adapted to try to make the total number of species come out correctly (i.e. \code{spcat} will be set to \code{totsp/(n.site*n.abund)}.  If \code{totsp} is not an even multiple of \code{n.site*n.abund}, the results may be slightly unpredictable \ldots
<<diffsites,fig.width=12,out.width="1.2\\textwidth">>=
plot(betasim(n.site=5,n.abund=3,p.mix=0.25,rand="poisson"))
@

\code{calcbeta} calculates distances from the centroid (by default,
using a Jaccard index on presence-absence data):
<<centrdisthist,cache=TRUE>>=
...
\cite{legendre_2013}; however, \cite{legendre_2013} have also criticized the Manhattan index,
